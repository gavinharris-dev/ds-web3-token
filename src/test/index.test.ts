...
test('If you mess with the signed token, you cannot validate', async () => {
  ...
  await expect(verify(`${signedMsg.substring(0, 20)}INJECT${signedMsg.substring(20)}`)).rejects.toThrow();
});
...
export const signDataCIP30 = async (msg: string) => {
  ...
  const wallet = Loader.Cardano.Bip32PrivateKey.from_bip39_entropy(Buffer.from(entropy, 'hex'), Buffer.from(''));
  ...
  protectedHeaders.set_algorithm_id(Loader.Message.Label.from_algorithm_id(Loader.Message.AlgorithmId.EdDSA));
  ...
  const protectedSerialized = Loader.Message.ProtectedHeaderMap.new(protectedHeaders);
  const unprotectedHeaders = Loader.Message.HeaderMap.new();
  const headers = Loader.Message.Headers.new(protectedSerialized, unprotectedHeaders);
  const builder = Loader.Message.COSESign1Builder.new(headers, Buffer.from(msg, 'hex'), false);
  const toSign = builder.make_data_to_sign().to_bytes();

  const signedSigStruc = paymentKey.sign(toSign).to_bytes();
  const coseSign1 = builder.build(signedSigStruc);

  const key = Loader.Message.COSEKey.new(Loader.Message.Label.from_key_type(Loader.Message.KeyType.OKP));
  key.set_algorithm_id(Loader.Message.Label.from_algorithm_id(Loader.Message.AlgorithmId.EdDSA));
  key.set_header(
    Loader.Message.Label.new_int(Loader.Message.Int.new_negative(Loader.Message.BigNum.from_str('1'))),
    Loader.Message.CBORValue.new_int(
      Loader.Message.Int.new_i32(6), // Loader.Message.CurveType.Ed25519
    ),
  ); // crv (-1) set to Ed25519 (6)
  key.set_header(
    Loader.Message.Label.new_int(Loader.Message.Int.new_negative(Loader.Message.BigNum.from_str('2'))),
